...
2. 页面固定使用 **main** 通讯格式，无需选择固件；当前 SoftEngine 固件兼容这一单块短命令格式。
3. 关闭 SIM，进入连接页，对照扫描区域右侧的名称提示框，搜索 **D30SP_126BB2** 并点击对应设备的「连接」。
4. 连接后自动选择 FFE0 服务的 **FFE2** 写入通道，并订阅 **FFE1** 返回通知，页面显示「通讯已就绪」，无需手动选择。若缺少可写 FFE2，连接失败并提示，不回退到 FFE1 写入。
5. 启用本地控制，再操作摇杆；松手归零，断连后固件在 500 ms 超时归零。

当前 main 命令最多 20 字节，每条完整写入一次，命令间隔至少 25 ms，100 ms 周期更新控制目标。
